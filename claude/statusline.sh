...
# Line 2: current: <progressbar> % | extra: <progressbar> $used/$limit
# Line 3: resets <time> | resets <date>
...
if [ -n "$usage_data" ] && echo "$usage_data" | jq -e . >/dev/null 2>&1; then
    ...
    line1+=" ${dim}|${reset} "
    line1+="${white}limit:${reset} ${five_hour_bar} ${cyan}${five_hour_pct}%${reset}"
    line1+=" ${dim}resets ${five_hour_reset}${reset}"
fi
